  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-card rounded-lg border border-border hover:bg-muted transition-colors">
      ...
      <div className="flex-1">
        <span className={`block ${todo.completed ? "line-through text-muted-foreground" : "text-foreground"}`}>
        ...
        <div className="flex gap-2.5 mt-1 flex-wrap">
          ...
          {todo.tags.map((tag, index) => (
            <span key={`${todo.id}-${tag}-${index}`} className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded">
              {tag}
            </span>
          ))}
        </div>
